...
class TestReasoningGymGameOfLife(unittest.TestCase):
    def test_game_of_life_single_turn_env(self):
        # Save reset image for quick manual inspection/debugging.
        output_dir = (
            Path(__file__).resolve().parent / "test_output_reasoning_gym_game_of_life"
        )
        ...
        env_id = "ReasoningGym/GameOfLife-v0"
        ...
        print("\n" + "=" * 80)
        print("[ReasoningGym/GameOfLife-v0] QUESTION:\n")
        print(obs.text[:500] if obs.text else "No text")
        print("\n[ReasoningGym/GameOfLife-v0] ORACLE ANSWER:\n")
        print(oracle[:200] + "..." if len(oracle) > 200 else oracle)
        print("=" * 80 + "\n")
