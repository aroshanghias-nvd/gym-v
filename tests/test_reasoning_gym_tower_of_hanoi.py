...
class TestReasoningGymTowerOfHanoi(unittest.TestCase):
    def test_tower_of_hanoi_single_turn_env(self):
        # Save reset image for quick manual inspection/debugging.
        output_dir = (
            Path(__file__).resolve().parent / "test_output_reasoning_gym_tower_of_hanoi"
        )
        ...
        env_id = "ReasoningGym/TowerOfHanoi-v0"
        ...
        print("\n" + "=" * 80)
        print("[ReasoningGym/TowerOfHanoi-v0] QUESTION:\n")
        print(obs.text[:500] if obs.text else "No text")
        print("\n[ReasoningGym/TowerOfHanoi-v0] ORACLE ANSWER:\n")
