...
class Chessboard:
    ...
    def __init__(self, size: int = 5, *, rng: random.Random | None = None):
        self.size = size
...
class Jewel2QAEnv(Env):
    ...
    def _load_element_images(self) -> None:
        ...
        images_dir = (
            resources.files("gym_v.envs") / "assets" / "gamerl" / "jewel2" / "images"
        )
            ...
            for c in range(self.size):
                ...
                if element in self._element_images:
                    ...
                    if element != " ":
                        ...
                        draw.text(
                            (text_x, text_y), element, fill="white", font=elem_font
                        )
                    else:
                        # Draw empty cell
                        draw.rectangle(
    ...
    def _generate_question(self, question_type: int) -> None:
        """Generate question of specified type - sets _question, _options, _oracle_answer"""
        if question_type == 0:
...
{grid_str}"""
    ...
    def reset(
        ...
        if self._question_type_param is not None:
            ...
            self._question_type_idx = int(self.np_random.integers(0, 7))
