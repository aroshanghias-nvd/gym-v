...
class HueQAEnv(Env):
    # Meta: source=GameRL, category=cognition, turn=single
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._board_size_param = board_size
        self._num_lines_param = num_lines
        self._board_size = board_size if board_size is not None else 5
        self._num_lines = num_lines if num_lines is not None else 3
    ...
    def reset(
        ...
        self._board_size = (
            self._board_size_param
            if self._board_size_param is not None
            else self.py_random.randint(5, 8)
        )
        self._num_lines = (
            self._num_lines_param
            if self._num_lines_param is not None
            else self.py_random.randint(3, 4)
        )
    ...
    def _generate_color_description_question(self) -> dict[str, Any]:
        ...
        options = sorted(wrong_descriptions) + [correct_description]
    ...
    def _generate_gradient_pattern_question(self) -> dict[str, Any]:
        ...
        options = sorted(wrong_descriptions) + [correct_desc]
        self.py_random.shuffle(options)
    ...
    def _generate_color_matching_question(self) -> dict[str, Any]:
        ...
        self._removed_positions = self.py_random.sample(
            sorted(self._colored_cells), num_removed
        )
        removed_colors = []
